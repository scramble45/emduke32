...
ifeq ($(EMSCRIPTEN),1)
    # OPTOPT=-msimd128 -msse -msse2 -msse3 -mssse3
    # SDL_TARGET:=2
    SDLCONFIG:=
	STRIP:=
	EMPRELOAD:=
	...
	WITHOUT_GTK:=1
	OPTLEVEL:=3
	EMFLAGS:= -s USE_LIBPNG=1 -s USE_ZLIB=1 -s USE_OGG=1 -s USE_VORBIS=1 -s USE_PTHREADS=1 -s USE_SDL=2 -s USE_SDL_MIXER=2 -s TOTAL_MEMORY=268435456
	ifeq ($(HTML),1)
		EXT=.html
	else
		EXT=.js
